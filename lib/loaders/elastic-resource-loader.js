...
class ElasticResourceLoader extends AbstractResourceLoader {
    ...
    async begin() {
        ...
        this.indexName = `${this.aliasName}_${timestamp}`;

        try {            
            await this.estools.createIndex(this.indexName, this.mappings, this.settings);
        } catch (err) {
    ...
    /**
     * Cleans up all the old unused indices
     * NOTE: this does not handle minimum to keep yet.
     */
    async cleanupOldIndices() {
        try {
            //Setup time for the old date.
            const olderThanDate = moment().subtract(this.daysToKeep, 'days').startOf('day').valueOf();
            ...
            const aliasedIndices = await this.estools.getIndicesForAlias(this.aliasName);
            ...
            if (indicesToDelete) {
                await Promise.all(
                    indicesToDelete.map(
                        async (name) => {
                            await this.estools.deleteIndex(name);
                        }
                    )
                );
            }
