...
class NetlifyMdResourceTransformer extends AbstractResourceTransformer {
    ...
    mapFacet(facet, incoming) {
        ...
        return incoming[facet].map((tag) => {
            const pair = this.facetMaps[facet][tag];
            if (!pair) {
                console.warn(`Could not lookup key ${tag} for facet ${facet}`);
    ...
    async transform(data) {
        ...
        const resource = {
            ...
            toolTypes: this.mapFacet("toolTypes", document.data),
        }
        return document;
    }
